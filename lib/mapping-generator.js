...
function getMapping (cleanTree, inPrefix) {
  ...
  for (field in cleanTree) {
    ...
    // if type is mixed, delete type, but keep field name
    // so that serialize.js will index the field
    if (mapping[field].type === 'mixed') {
      delete mapping[field].type
    } else if (mapping[field].type === undefined) {
      // else if type is never mapped, delete mapping
