...
// @return the mapping
//
function getMapping(cleanTree, schema, inPrefix) {
  var mapping = {},
    value, field, prop,
    implicitFields = [],
    hasEsIndex = false,
  ...
  for (field in cleanTree) {
    ...
    if (value.es_indexed) {
      hasEsIndex = true;
    } else if (value.type && !schema.nested[field]) {
      implicitFields.push(field);
    }

    // If there is no type, then it's an object with subfields.
    if (typeof value === 'object' &&
          (!value.type || schema.nested[field] || value.es_type === 'nested') &&
          value.es_type !== 'geo_shape') {
      mapping[field].type = 'object';
      mapping[field].properties = getMapping(value, schema, prefix + field);
    }
...
Generator.prototype.generateMapping = function generateMapping(schema, cb) {
  ...
  mapping = getMapping(cleanTree, schema, '');
  cb(null, {
    properties: mapping
  });
};
