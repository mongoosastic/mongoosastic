...
function Mongoosastic (schema, pluginOpts) {
  ...
  let bulkBuffer = []
  let esClient
  let mongoose = options && options.mongoose
  let populate = options && options.populate
  let populateChildren = options && options.populateChildren
  ...
  function postSave (doc) {
    ...
    function postPopulate (err, popDoc) {
      if (err) {
        // console.error('ERROR: Failed to populate children documents. Error:', err);
        onIndex(err, popDoc);
      } else {
    ...
    function postPopulateChildren (err, popChildDoc) {
      if (err) {
        // console.error('ERROR: Failed to populate documents. Error:', err);
        postPopulateChildren(err);
      } else {
        if (populate && populate.length) {
          populate.forEach(populateOpts => {
            popChildDoc.populate(populateOpts)
          })
          popChildDoc.execPopulate().then(popChildDoc => {
            postPopulate(null, popChildDoc);
          }).catch(postPopulate)
        } else {
          postPopulate(null, popChildDoc);
        }
    ...
    if (doc) {
      ...
      if (populateChildren && populateChildren.length) {
        _doc.populateChildren(populateChildren).then((popChildDoc) => {
          postPopulateChildren(null, popChildDoc);
        }).catch(postPopulateChildren);
      } else {
        postPopulateChildren(null, _doc);
      }
  ...
  schema.methods.populateChildren = function populateChildren (inOpts) {
    let promises = [],
      opts = inOpts;
    opts.forEach(opt => {
      promises.push(this.populateChild(opt));
    });
  ...
  schema.methods.populateChild = function populateChild (inOpts) {
    let opts = inOpts,
      query = {};
    query[opts.childKey] = {
      $in: this.get('_id')
    };
    return mongoose.model(opts.ref).find(query).lean().distinct('_id').then(childDocIds => {
      this[opts.key] = childDocIds;
    });
  };
