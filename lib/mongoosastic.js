...
module.exports = function Mongoosastic(schema, options){
  ...
  */
  ...
  schema.statics.search = function(query, options, cb){
    ...
    query.index = options.index || indexName
    query.type  = options.type  || typeName
  ...
  function bulkDelete(indexName, typeName, model, cb) {
  ...
  function setUpMiddlewareHooks(schema) {
    schema.post('remove', function(done){
      setIndexNameIfUnset(this.constructor.modelName)
      ...
      if(bulk) {
        bulkDelete(options, done)
      } else {
        deleteByMongoId(options, done)
      }
    })
    ...
    schema.post('save', function(done){
      this.index(function(err, res){
        this.emit('es-indexed', err, res)
        done(err)
      })
    })
  }
...
function deleteByMongoId(options, cb){
  ...
  client.delete(index, type, model._id.toString(), function(err, res){
