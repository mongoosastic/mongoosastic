...
function hydrate(res, model, options, cb) {
  ...
  query.exec(function queryCb(err, docs) {
    ...
    if (err) {
      ...
      hydrateDefer.resolve(res);
    }
  });
  return hydrateDefer.promise;
}
...
function Mongoosastic(schema, pluginOpts) {
   ...
   * @returns {Promise} - a promise to query resute
   */
  schema.statics.search = function search(inQuery, inOpts, inCb) {
    ...
    function searchCb(err, res) {
      ...
      if (alwaysHydrate || opts.hydrate) {
        result = hydrate(res, _this, opts, cb);
      } else {
        if (cb) {
          cb(null, res);
        }
...
//  cb ? searchCb : undefined
    return esClient.search(esQuery).then(function (res) {
      return searchCb(null, res);
    }).catch(function (err) {
      if (cb) {
        searchCb(err);
      }
