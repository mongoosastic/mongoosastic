...
function createMappingIfNotPresent(options, cb) {
  ...
  generator.generateMapping(schema, (ignoredErr, mapping) => {
    ...
    if (properties) {
      Object.keys(properties).map(key => { // eslint-disable-line array-callback-return
...
function Mongoosastic(schema, pluginOpts) {
   ...
   * @param cb Function
   */
  function createMapping(inSettings, inCb, ...args) {
    var cb = inCb,
      settings = inSettings;
    if (args.length < 2) {
      cb = inSettings || nop;
  ...
  schema.statics.createMapping = createMapping;

  /**
   * @param options  Object (optional)
   * @param cb Function
   */
  function schemaIndex(inOpts, inCb, ...args) {
    var index,
      type,
      serialModel,
      cb = inCb,
  ...
  schema.methods.index = schemaIndex;
   ...
   */
  function unIndex(inOpts, inCb, ...args) {
    var opts = inOpts,
      cb = inCb;
   ...
   */
  function esTruncate(inOpts, inCb, ...args) {
    var opts = inOpts,
      cb = inCb,
      esQuery;
  ...
  schema.statics.esTruncate = esTruncate;
   ...
   */
  function synchronize(inQuery, inOpts) {
  ...
  schema.statics.synchronize = synchronize;
   ...
   */
  function search(inQuery, inOpts, inCb, ...args) {
    var cb = inCb,
      opts = inOpts,
    ...
    esSearchBind = schema.statics.esSearch.bind(this);

    return esSearchBind(fullQuery, opts, cb);
  }
   ...
   * @param inCb - callback called with search results
   */
  function esSearch(inQuery, inOpts, inCb, ...args) {
    var _this = this,
      cb = inCb,
      opts = inOpts,
      esQuery,
  ...
  schema.statics.flush = flush;

  function refresh(inOpts, inCb, ...args) {
    var cb = inCb,
      opts = inOpts;
    if (args.length < 2) {
      cb = inOpts || nop;
  ...
  schema.statics.refresh = refresh;
