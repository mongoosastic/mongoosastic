  ...
  */
  ...
  function setUpRiver(schema) {
    schema.statics.river = function(cb) {
      ...
      // the river definition can come from the options of mongoosasic,
      // but some attributes will be overwritten anyway
      // see https://github.com/richardwilly98/elasticsearch-river-mongodb/wiki
      var riverDefinition = useRiver.definition || {};

      riverDefinition.type = 'mongodb';
      riverDefinition.mongodb = riverDefinition.mongodb || {};
      riverDefinition.mongodb.db = this.db.name;
      riverDefinition.mongodb.collection = this.modelName.toLowerCase() + 's';
      riverDefinition.gridfs = (useRiver && useRiver.gridfs) ? useRiver.gridfs : false;
      riverDefinition.index = riverDefinition.index || {};
      riverDefinition.index.name = indexName;
      riverDefinition.index.type = typeName;

      esClient.putRiver('mongodb', indexName, riverDefinition, cb);
    }
  }
};
