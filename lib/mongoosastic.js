...
function Mongoosastic(schema, options) {
	...
	schema.methods.index = function (options, cb) {
		...
		if (bulk) {
			...
			serialModel[modelFieldName] = this.constructor.modelName;
			esClient.index({
				index: index,
				type: type,
		...
		esClient.search(esQuery, function (err, res) {
			if (err) {
				...
				if (alwaysHydrate || options.hydrate) {
					...
					options.modelFieldName = modelFieldName
					hydrate(res, model, options, cb)
				} else {
					cb(null, res)
				}
			}
...
module.exports = {
	...
	search:function(query, options, cb){
		...
		esClient.search(esQuery, function (err, res) {
			if (err) {
				...
				if (options.hydrate) {
					options.modelFieldName = options.modelFieldName || _this.options.modelFieldName
					hydrate(res,null,options || {}, cb)
				} else {
					cb(null, res)
				}
			}
	...
	plugin: function(options){
		var _this = this;
		options = options || {};
		options.host = options && options.host ? options.host : 'localhost' 
		options.protocol = options && options.protocol ? options.protocol : 'http'
		options.auth = options && options.auth ? options.auth : null
		options.bulk = options && options.bulk
		options.modelFieldName = options && options.modelFieldName ? options.modelFieldName : "__model_name__"
		return function(schema,_options){
			_this.options = extend(options,_options);
			return Mongoosastic(schema,_options);
		}
	}
};
...
function hydrate(res, model, options, cb) {
	...
	Object.keys(options).forEach(function (option) {//想不到这个可以支持populate,真让我非常开心啊
	...
	query.exec(function (err, docs) {
		if (err) {
			...
			docs.forEach(function (doc) {
				var i = resultsMap[doc._id] //按照es输出的顺序来从新组合数据
				hits[i] = doc
			})
			results.hits = hits
			res.hits = results
...
function hydrate(res,model, options, cb) {
	...
	results.hits.forEach(function(a,i){
		...
		if(source[modelFieldName]) {
			resultsMap[source[modelFieldName]] = resultsMap[source[modelFieldName]] || {};
			ids[source[modelFieldName]] = ids[source[modelFieldName]] || [];
			
			
			resultsMap[source[modelFieldName]][a._id] = i;//记录排序索引
			ids[source[modelFieldName]].push(a._id);
		}
	});
